...
//#include "delay.h"
/*==========================================================================*/
/* 微秒级延时函数 (针对72MHz主频校准, 每循环约10个指令周期)                */
/*==========================================================================*/
...
//OLED反色显示/正常显示
void OLED_DisplayInvert(unsigned char invert)
{
	if(invert)
		OLED_WR_Byte(0xA7, OLED_CMD);  // 反色显示(夜间模式)
	else
...
void OLED_DrawDinoDuck(void)
{
	static unsigned char duck_dir = 0;
	unsigned int j = 0;
	unsigned char x, y;
	unsigned char byte;
...
int OLED_DrawBird(unsigned char reset)
{
	static int bird_pos = 128;
	static unsigned char bird_anim = 0;
	static unsigned char bird_height = 0;
	char speed = 5;
	int start_x = 0;
	unsigned char x, y;
	unsigned char byte;
	...
	if (reset == 1)
	{
		...
		bird_anim = 0;
		bird_height = 0;
		oled_drawbmp_block_clear(0, 2, speed);
		return 128;
	}

	bird_anim = (bird_anim + 1) % 4;
	...
	for (y = 0; y < 2; y++)
	{
		...
		IIC_WriteByte(0x40);
		IIC_WaitAck();
		for (x = start_x; x < 16; x++)
		{
			if (bird_pos + x > 127) break;
			j = y * 16 + x;
			if (bird_anim < 2)
				byte = BIRD[y][x];
			else
	...
	oled_drawbmp_block_clear(bird_pos + 16, 2, speed);
	bird_pos = bird_pos - speed;
	return bird_pos + speed;
}
